...
- 価格は **list price ベースの概算**。真値は Billing Export 側。`costs.py` の価格表は `conf/project.yaml` の `jpyPerUsd` で円換算。
...
- 2026-06-30 **Discord 通知**追加: `make cost-notify`（当月サマリ送信）＋ `cost-record` 時に当月累計が ¥1000 以上なら自動通知。webhook は `conf/secret.yaml`（gitignore 済、`conf/secret.example.yaml` にテンプレ）/ env `DISCORD_WEBHOOK_URL` でも可。**ハマり所**: Discord は urllib 既定 UA を 403 で弾くため `User-Agent` ヘッダ必須（修正済、実送信成功）。
