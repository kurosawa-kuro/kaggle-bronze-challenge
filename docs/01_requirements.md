...
- 本番 MLOps 水準のアーキテクチャ（Port/Adapter の多層化, KFP パイプライン本格運用, Endpoint / Model Registry / Monitoring は使わない）
  - ただし Vertex AI **Custom Job は「並列実験ランナー」として使う**（本番基盤としてではない → `docs/adr/0001-vertex-ready-experiment-runner.md`）
...
--- 使わないもの ---
Turso          : SQLite で十分。サーバーレス DB は不要
Ray / Spark    : ローカル単機 + Vertex Custom Job で十分。分散基盤は不要
dbt            : SQL 変換ツール。EDA には DuckDB 直クエリで足りる
MLflow / wandb : SQLite + DuckDB + run_id 成果物で代替。外部サービス登録コスト不要
Vertex の本格機能 : Feature Store / Pipelines 本格運用 / Endpoint / Model Registry / Monitoring は使わない
```
