...
ただし、**現在コードで実体化済みなのは Custom Job / Hyperparameter Tuning / Model Registry / Pipelines(KFP) / GCS / Artifact Registry / BigQuery ログ・コスト記録**である（Pipelines は compile 検証済み、実 run は image 再 push 前提）。Endpoint / Monitoring / Batch Prediction は ADR 上の拡張候補であり、実推論コンテナが必要なため未着手。
